...
public class SubscriptionController {
    ...
    public String addSubscriptionsShowPage(ModelMap model) {
        model.addAttribute("subscription", new SubscriptionPojo(0, "Tier", 
        0.0, 0, "Accessibility"));
        return "addSubscription";
    }

    @PostMapping("/add-subscription")
    public String addSubscriptions(SubscriptionPojo subscription) {
        services.addSubscription(subscription.getSubsc_tier(), subscription.getSubsc_fee(),
                subscription.getSubsc_limit(), subscription.getSubsc_accessibility());
    ...
    public String editSubscriptionDetails(int id, @RequestParam String tier, @RequestParam double fee,
            @RequestParam int limit, @RequestParam String accessibility) {
